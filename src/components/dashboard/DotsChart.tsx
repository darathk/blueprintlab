...
const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    return (
        <div style={{ background: 'rgba(15,23,42,0.95)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '10px 14px', backdropFilter: 'blur(10px)', fontSize: 12 }}>
            <p style={{ fontWeight: 700, color: '#f8fafc', marginBottom: 6 }}>{label}</p>
            {payload.map((p: any) => (
                <p key={p.dataKey} style={{ color: p.color, margin: '2px 0' }}>
                    {p.name}: <strong>{p.value}{p.dataKey === 'dots' ? '' : ' lbs'}</strong>
                </p>
            ))}
        </div>
    );
...
export default function DotsChart({ athleteId, logs, initialGender, initialWeightClass }: Props) {
    ...
    const data: CompetitionDataPoint[] = getCompetitionDataPoints(logs, wc, genderKey);
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{ background: 'rgba(15,23,42,0.5)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '16px 20px' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'flex-end' }}>
                            ...
                            {['male', 'female'].map(g => (
                                <button
                                    ...
                                    style={{
                                        ...
                                        color: gender === g ? '#fff' : 'var(--secondary-foreground)',
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                                {classes.map(c => (
                                    <button
                                        key={c}
                                        onClick={() => setWeightClass(c === 'Open' ? '140' : c.toString())}
                                        style={{
                                            padding: '5px 12px', borderRadius: 6, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600,
                                            background: weightClass === (c === 'Open' ? '140' : c.toString()) ? 'var(--primary)' : 'rgba(255,255,255,0.07)',
                                            color: weightClass === (c === 'Open' ? '140' : c.toString()) ? '#fff' : 'var(--secondary-foreground)',
                                            transition: 'all 0.15s',
                                        }}
                                    >
                    ...
                    <button
                        onClick={handleSave}
                        disabled={saving || !gender || !weightClass}
                        style={{
                            padding: '8px 20px', borderRadius: 8, border: 'none', cursor: gender && weightClass ? 'pointer' : 'not-allowed', fontSize: 13, fontWeight: 700,
                            background: saved ? 'var(--success)' : 'linear-gradient(135deg, #7d87d2, #a855f7)',
                            color: '#fff', opacity: (!gender || !weightClass) ? 0.4 : 1,
            ...
            {data.length > 0 && latestTotal > 0 && (
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    {[
                        { label: 'Latest DOTs', value: latestDots > 0 ? latestDots.toFixed(1) : '—', sub: 'score', color: CHART_COLORS.dots },
                        { label: 'Best Squat E1RM', value: `${Math.max(...data.map(d => d.squat))} lbs`, color: CHART_COLORS.squat },
                        { label: 'Best Bench E1RM', value: `${Math.max(...data.map(d => d.bench))} lbs`, color: CHART_COLORS.bench },
                        { label: 'Best Deadlift E1RM', value: `${Math.max(...data.map(d => d.deadlift))} lbs`, color: CHART_COLORS.deadlift },
                    ].map(s => (
            ...
            {data.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--secondary-foreground)', fontSize: 14, background: 'rgba(15,23,42,0.3)', borderRadius: 12, border: '1px dashed rgba(255,255,255,0.08)' }}>
                    ...
                    <div style={{ fontWeight: 600, marginBottom: 5 }}>No competition lift data yet</div>
                    <div style={{ fontSize: 12, opacity: 0.7 }}>Sessions with exact <strong>Squat</strong>, <strong>Bench Press</strong>, or <strong>Deadlift</strong> will appear here once logged with weight, reps & RPE.</div>
                </div>
            ) : (
                <div style={{ background: 'rgba(15,23,42,0.4)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.07)', padding: '16px 8px 8px' }}>
                    {/* Toggle buttons */}
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', paddingLeft: 12, marginBottom: 12 }}>
                        {[
                            { key: 'squat', label: 'Squat E1RM' },
                            { key: 'bench', label: 'Bench E1RM' },
                            { key: 'deadlift', label: 'Deadlift E1RM' },
                            { key: 'dots', label: 'DOTs Score' },
                        ].map(({ key, label }) => {
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                        <LineChart data={data} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                            ...
                            {activeLines.dots && genderKey && wc > 0 && <Line yAxisId="dots" type="monotone" dataKey="dots" name="DOTs Score" stroke={CHART_COLORS.dots} strokeWidth={2.5} dot={{ r: 4, fill: CHART_COLORS.dots }} activeDot={{ r: 6 }} strokeDasharray="none" />}
                        </LineChart>
                    </ResponsiveContainer>
                </div>
            )}
